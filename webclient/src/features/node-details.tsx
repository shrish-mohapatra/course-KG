...
const SAMPLE_NODE = {
  ...
  metadata: {
    sources: ["express-2 lecture slides.pdf", "express-2 transcript.csv"],
...
const NodeDetails = () => {
  ...
  return (
    ...
    >
      <div
        className="bg-black w-[420px] min-h-96 rounded-[24px] p-2 flex flex-col gap-1"
        ref={modalRef}
      >
        ...
        <div className="flex flex-col flex-grow gap-1 bg-background rounded-[24px] p-4">
          ...
          <div>
            {selectedNode.sources.map((source, index) => (
              <p className="text-accent underline" key={`source-${index}`}>
                {renderSource(source)}
              </p>
            ))}
          </div>
